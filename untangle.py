...
class det:
    ...
    def doIntersect(p1, q1, p2, q2):
        ...
        if o1 == 0 and det.onSegment(p1, p2, q1):
            return True

        # p1, q1 and q2 are colinear and q2 lies on segment p1q1
        if o2 == 0 and det.onSegment(p1, q2, q1):
            return True

        # p2, q2 and p1 are colinear and p1 lies on segment p2q2
        if o3 == 0 and det.onSegment(p2, p1, q2):
            return True

            # p2, q2 and q1 are colinear and q1 lies on segment p2q2
        if o4 == 0 and det.onSegment(p2, q1, q2):
            return True
...
NODES = {"easy": 10, "normal": 20, "hard": 50}
DEFAULTNODES = 20
DEFAULTANIMAT = True
...
class Game:
    ...
    def __init__(
        self, clock=None, nodes=None, anim=DEFAULTANIMAT, difficulty=DEFAULTDIFFICULTY
    ):
        if nodes is None:
        ...
        self.blockSize = 14
        self.gridSize = (
            1  # self.blockSize /2 +1 #rougher grid makes it a cluncky to use
        )
    ...
    def printGameStats(self):
        if self.gameState == RUN:
            ...
            print(
                "Untangled {0}/{1} in {2} with {3} moves".format(
                    self.untangledCount, len(self.rects), self.gameduration, self.moves
                ),
                end="\r",
            )
    ...
    def checkRanking(self):
        ...
        if rank["moves"] >= 0 or rank["time"] >= 0:
            # Ranked so get name
            name = input(
                "Congratulations! You have a new high score! Please enter your name: "
            )
    ...
    def printRankings(self, rankings):
        ...
        for index in range(0, len(rankings["moves"])):
            print(
                "{0} -- {1} - {2} -- {3}".format(
                    index + 1,
                    rankings["moves"][index]["score"],
                    rankings["moves"][index]["name"],
                    rankings["moves"][index]["occurred"],
                    end="\n",
                )
            )
        ...
        for index in range(0, len(rankings["time"])):
            print(
                "{0} -- {1} - {2} -- {3}".format(
                    index + 1,
                    rankings["time"][index]["score"],
                    rankings["time"][index]["name"],
                    rankings["time"][index]["occurred"],
                    end="\n",
                )
            )
    ...
    def createCirclePoints(
        self, points, radius, center=(int(SCREEN_WIDTH / 2), int(SCREEN_HEIGHT / 2))
    ):
        """
    ...
    def testSegmentIntercet(self, i, c):
        ...
        for j in range(0, len(self.rects)):  # other nodes...
            for d in self.rects[j][1] + self.rects[j][2]:  # ... ande their connections
                # print (i, c, j, d)
                if (
                    j == i or j == c or d == i or d == c
                ):  # but not the node or its connections itself
                    pass
                elif det.doIntersect(
                    self.rects[i][0],
                    self.rects[c][0],
                    self.rects[j][0],
                    self.rects[d][0],
    ...
    def placeOnCircle(self, rects):
        ...
        circlePositions = self.createCirclePoints(
            len(rects),
            min(
                len(rects) * 5,
                int(SCREEN_WIDTH / 2) - self.blockSize,
    ...
    def createRandomConnections(self):
        ...
        while self.placeNodes and (not self.showBuildAnimation or once):
            ...
            for i, r in enumerate(self.rects):
                r[0].topleft = (
                    random.randint(0, SCREEN_WIDTH - self.blockSize),
    ...
    def buildNodesConnection(self, rects):
        ...
        while self.placeNodes == 3 and (not self.showBuildAnimation or once):
            ...
            rects[currNode][0].center = (
                40 + i * 80 + self.placeCount % 2 * 20 + random.randint(-10, 10),
            ...
            if self.placeCount >= len(rects):
                ...
                if j != 0:
                    ...
                    while connections and tries:
                        connection = random.randint(
                            max(0, currNode - int(groupMembers * 1.5)), currNode - 2
                        )
                        if (
                            connection != currNode
                            and not connection in rects[currNode][1]
                            and not self.testSegmentIntercet(currNode, connection)
                        ):
    ...
    def buildPuzzleConnection(self, rects):
        ...
        while self.placeNodes == 3 and (not self.showBuildAnimation or once):
            ...
            if len(rects) < 6:
                ...
                layerDiameterIncrease = min(
                    (SCREEN_HEIGHT / 2 - 2 * self.blockSize) / (len(rects) / ammount),
                    50,
                )
            ...
            if (
                not hasattr(self, "positionList") or not self.positionList
            ):  # create positions for nodes, do only once
                self.positionList = self.createCirclePoints(
                    ammount
                    if self.curLayer < layers - 1
                    else len(rects) - self.placeCount,
            ...
            while connections and tries:
                ...
                if (
                    connection != self.placeCount
                    and not connection in rects[self.placeCount][1]
                    and not self.testSegmentIntercet(self.placeCount, connection)
                ):
        ...
        # ---------------------------------------------------------------------
            ...
            if event.button == 1:  # select
                for i, r in enumerate(self.rects):
                    ...
                    if (
                        distance_square <= int(self.blockSize / 2) ** 2
                    ):  # c^2 <= radius^2
                        self.selected = i
                        self.selected_offset_x = (
                            r[0].x - event.pos[0]
                        )  # makes the cursor drag at current position
                        self.selected_offset_y = (
                            r[0].y - event.pos[1]
            ...
            if event.button == 1 and self.selected is not None:
                self.rects[self.selected][0].x = self.myRound(
                    self.rects[self.selected][0].x
                )
                self.rects[self.selected][0].y = self.myRound(
                    self.rects[self.selected][0].y
                )
            ...
            if (
                event.buttons[2] and self.gameState >= RUN
            ):  # drag all nodes across with RMB
                for rect in self.rects:
                    rect[0].x += event.rel[0]
                    rect[0].y += event.rel[1]
            elif (
                self.selected is not None
            ):  # selected can be `0` so `is not None` is required
                # move object
                self.rects[self.selected][0].x = self.myRound(
                    event.pos[0] + self.selected_offset_x
                )
                self.rects[self.selected][0].y = self.myRound(
                    event.pos[1] + self.selected_offset_y
                )
    ...
    def gameLoop(self):
        ...
        if self.tileMoved is not False:  # test only moved node (can be 0) ...
            ...
            for c in self.rects[i][1] + self.rects[i][2]:  # ...has connections to
                ...
                for j in range(0, len(self.rects)):  # other nodes...
                    for d in (
                        self.rects[j][1] + self.rects[j][2]
                    ):  # ... ande their connections
                        # print (i, c, j, d)
                        if (
                            j == i or j == c or d == i or d == c
                        ):  # but not the node or its connections itself
    ...
    def draw(self, screen):
        ...
        for r in self.rects:
            for l in r[1]:
                pygame.draw.line(
                    screen,
                    self.colorPalette["line"],
                    r[0].center,
                    self.rects[l][0].center,
                    2,
                )
        ...
        if self.selected is not None:
            pygame.draw.circle(
                screen,
                self.colorPalette["selected"],
                self.rects[self.selected][0].center,
                int(self.blockSize / 2),
            )
            for r in self.rects[self.selected][1] + self.rects[self.selected][2]:
                pygame.draw.circle(
                    screen,
                    self.colorPalette["selectedConnecting"],
                    self.rects[r][0].center,
                    int(self.blockSize / 2),
                )
        ...
        if self.mark is not None:
            pygame.draw.circle(
                screen,
                self.colorPalette["marked"],
                self.rects[self.mark][0].center,
                int(self.blockSize / 2),
                2,
            )
            for r in self.rects[self.mark][1] + self.rects[self.mark][2]:
                pygame.draw.circle(
                    screen,
                    self.colorPalette["markedConnecting"],
                    self.rects[r][0].center,
                    int(self.blockSize / 2),
                    2,
                )
