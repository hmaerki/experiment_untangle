...
HIGH_SCORES_TEMPLATE: Dict[str, Dict[str, Any]] = {
    "easy": {"moves": [], "time": []},
    "normal": {"moves": [], "time": []},
    "hard": {"moves": [], "time": []},
}
...
def save(score, gamemode, position, name):
    ...
    if position["moves"] >= 0:
        # Moves high score
        highScore = {"name": name, "score": score["moves"], "occurred": datetime.now()}
        highscores[gamemode]["moves"].insert(position["moves"], highScore)

    if position["time"] >= 0:
        # Moves high score
        highScore = {"name": name, "score": score["time"], "occurred": datetime.now()}
    ...
    if len(highscores[gamemode]["moves"]) >= MAX_RANKING_POSITIONS:
        highscores[gamemode]["moves"] = highscores[gamemode]["moves"][
            :MAX_RANKING_POSITIONS
        ]

    if len(highscores[gamemode]["time"]) >= MAX_RANKING_POSITIONS:
        highscores[gamemode]["time"] = highscores[gamemode]["time"][
            :MAX_RANKING_POSITIONS
        ]
